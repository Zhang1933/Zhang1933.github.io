...
调试发现他返回的是fake_freelist[4]的内存位置,tcache bin补充的顺序是stack_buffer_1,stack_buffer_2,fake_freelist[0]...fake_freelist[4],正好7个,头插加入tcachbin。samllbin每次从尾部(i.e. samllbin头中的bk)取。
